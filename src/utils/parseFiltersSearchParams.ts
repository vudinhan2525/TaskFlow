...
export function parseFiltersSearchParams(filtersSearchParams: string): FiltersSearchParams {
  ...
  if (!filtersSearchParams) return result;
    ...
    for (const cond of conditions) {
      ...
      if (inMatch) {
        const key:keyof FiltersSearchParams = inMatch[1] as keyof FiltersSearchParams;
        const values = inMatch[2]
          .split(',')
          .map((v) => v.trim().replace(/^"(.*)"$/, '$1'));
        result[key] = values;
      } else if (eqMatch) {
        const key:keyof FiltersSearchParams = eqMatch[1] as keyof FiltersSearchParams;  
